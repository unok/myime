...
// PATCHED: Modified to avoid SIL memory lifetime error in Swift 6.2.1
public struct LeavesWithCommonPrefixIterator<T: Hashable> : Sequence, IteratorProtocol {
    var node: TrieNode<T>
    var text: any Sequence<T>
    var seq: [T] = []
    // PATCHED: Changed from lazy var to regular var and initialize immediately
    var iterator: AnyIterator<T>
    
    // PATCHED: Add explicit initializer
    init(node: TrieNode<T>, text: any Sequence<T>) {
        self.node = node
        self.text = text
        self.seq = []
        // PATCHED: Create AnyIterator wrapper to avoid type erasure issues
