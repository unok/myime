...
### キー割当のカスタマイズ（例: Ctrl+T で半角英数モードへ）

Mozc のキー設定で、任意のキーに入力モード切替を割り当てられる。例えば Ctrl+T で半角英数モードに切り替えると、続けて打ったキーがそのまま半角英字で入る。

1. 通知領域の Mozc アイコン →「プロパティ」→「一般」タブ →「キー設定の選択」の「編集」
2. 「編集」→「エントリーを追加」で次の2行を登録する（保存するとキー設定は「カスタム」になる）

| モード | 入力キー | コマンド |
|---|---|---|
| 入力文字なし | Ctrl t | 半角英数に入力切替 |
| 入力文字なし | Ctrl q | ひらがなに入力切替 |
